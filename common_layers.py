...
def embedding(x, vocab_size, embedding_size, name_scope='WordEmbedding'):
...
def domain_classifier(encoder_output, hidden_size):
  """Embed x of type int64 into dense vectors, reducing to max 4 dimensions."""
  with tf.variable_scope('DomainClassifier'):
    Wa = tf.get_variable('Wa', [hidden_size, hidden_size], initializer=\
                    tf.random_normal_initializer(0.0, hidden_size**-0.5, dtype=tf.float32))
    va = tf.get_variable('va', [hidden_size, 1], initializer=\
                    tf.random_normal_initializer(0.0, hidden_size**-0.5, dtype=tf.float32))
    shape = tf.shape(encoder_output)[:-1]
    encoder_output_flat = tf.reshape(encoder_output, [-1, hidden_size])
    energy = tf.matmul(tf.tanh(tf.matmul(encoder_output_flat, Wa)), va)
    energy = tf.reshape(energy, shape)
    alpha = tf.nn.softmax(energy)
    alpha = tf.expand_dims(alpha, 2)
    alpha = tf.tile(alpha, [1, 1, hidden_size])
    ctx = tf.multiply(alpha, encoder_output)
    ctx = tf.reduce_sum(ctx,1)
    Wf = tf.get_variable('Wf', [hidden_size, hidden_size], initializer=\
                    tf.random_normal_initializer(0.0, hidden_size**-0.5, dtype=tf.float32))
    out = tf.tanh(tf.matmul(ctx,Wf))
    return out
'''
...
def text_cnn_hidden_relu(inputs,
                     hidden_size,
                     output_size,
                     kernel_size=(1, 1),
                     second_kernel_size=(1, 1),
  ...
  pooled_outputs = []
  num_filters = output_size//len(filter_sizes)
  # inputs = tf.expand_dims(inputs, -1) # B,T,512
  for i, filter_size in enumerate(filter_sizes):
    with tf.name_scope("conv-maxpool-%s" % filter_size):
      filter_shape = [filter_size, output_size, num_filters]
      ...
      # h = tf.reduce_max(h, axis=2, name="pool", keep_dims=True)
      # h = tf.squeeze(h, 2) # B,T,512
      if dropout != 0.0:
        h = tf.nn.dropout(h, 1.0 - dropout)
      pooled_outputs.append(h)

  h_pool = tf.concat(pooled_outputs, 2) # B,T,512
  # return h_pool
  ret = conv1d(h_pool, output_size, second_kernel_size, name="conv2", **kwargs)
  return ret
